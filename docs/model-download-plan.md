...
## Goal
Replace the manual adb-push of the 272 MB `clauncher-gemma3-270m.task` model with an
...
- Repo: `findethics-labs/titan-launcher-gemma3-270m`  ← CONFIRM SPELLING (user typed
  "titan-laucnher-...", assumed typo of "launcher").
- Default upload artifact: `ml/export/clauncher-gemma3-270m.task` (`--task` overrides).
...
- Args: `--task ml/export/clauncher-gemma3-270m.task` (default), `--repo findethics-labs/titan-launcher-gemma3-270m`, `--public`, `--yes` (CI bypass of prompts).
...
- Writes/updates `model-manifest.json` on `main`:
  ```json
  { "version": "<commit-sha>",
    "url": "https://huggingface.co/findethics-labs/titan-launcher-gemma3-270m/resolve/<commit-sha>/clauncher-gemma3-270m.task",
    "sha256": "<hash>" }
  ```
...
- One hardcoded constant:
  `MANIFEST_URL = https://huggingface.co/findethics-labs/titan-launcher-gemma3-270m/resolve/main/model-manifest.json`
- Fetch + parse manifest → immutable `url` + expected `sha256`.
- Stream `url` to `clauncher-gemma3-270m.task.part`; resume via `Range: bytes=<partLen>-`
...
1. Confirm repo spelling: `titan-launcher-gemma3-270m` (assumed) vs `titan-laucnher-gemma3-270m` (as typed).
2. Verify HF `resolve/` URLs return HTTP Range support through the LFS CDN redirect (expected yes;
   validate during implementation).

## Reference context (as of planning)
- Model load path: `MediaPipeLlmInterpreter.modelFile()` = `getExternalFilesDir(null)/clauncher-gemma3-270m.task`; `isModelPresent()` gates the feature.
